...
def set_anclabels_discretized_phylo_tree(root):
    """
    In a discretized tree this fucntions sets label to each node 
    at each level based on level order traversal.
    This should be used only with discretized tree
    """
...
def get_spcname_to_anclabels(spcname_to_leafnode):
